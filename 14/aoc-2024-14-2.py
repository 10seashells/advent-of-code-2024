...
def print_map():
    map = np.zeros((map_size['y'],map_size['x']),dtype=int)
...
figx = 6
figy = 10
fig, axs = plt.subplots(figx,figy, figsize=(12,7))
map = []
for secs in range(0,10000):
    reset()
    for robot in robots:
        move(robot, secs)
    ...
    for robot in robots:
        map[robot['pos'][1]][robot['pos'][0]] +=1
    x = (secs%(figx*figy))//figy
    y = (secs%(figx*figy))%figy
    ax = axs[x][y]
    ax.imshow(map, interpolation='nearest')
    ax.set_title(secs)
    ax.set_axis_off()
    if (secs+1)%(figx*figy) == 0:
        fig.tight_layout()
        plt.show()
        fig, axs = plt.subplots(figx, figy,figsize=(12,7))
